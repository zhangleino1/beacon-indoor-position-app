		...
		methods: {
			...
			listenToBeaconUpdates() {
				uni.onBeaconUpdate((res) => {
					...
					this.updateLineChart();
                    // 在这里发送每个 beacon 的 RSSI 值到 Thingsboard
                    this.sendRSSIToThingsboard(res.beacons);
				});
			},
			calculatePosition(beacons) {
				if (beacons.length >= 3) {
					try {
						...
						this.updateScatterChart();
                         // 在这里发送定位坐标到 Thingsboard
                        this.sendPositionToThingsboard(position);
					} catch (e) {
						console.error(e);
					}
				} else {
            ...
            // 新增方法：发送定位坐标到 Thingsboard
            sendPositionToThingsboard(position) {
				debugger
                let telemetryData = {
                    x: parseFloat(position[0][0].toFixed(2)),
                    y: parseFloat(position[1][0].toFixed(2))
                };
                uni.request({
                    url: 'http://140.143.139.183:8080/api/v1/23WFRgK4lx1qq5QvrbDp/telemetry',
                    method: 'POST',
                    header: {
                    ...
                    data: telemetryData,
                    success: (res) => {
                        console.log('定位坐标发送成功', res.data);
                    },
                    fail: (err) => {
                        console.error('定位坐标发送失败', err);
                    }
                });
            },
            // 新增方法：发送每个 beacon 的 RSSI 值到 Thingsboard
            sendRSSIToThingsboard(beacons) {
				debugger
				console.info(11111111111)
                let telemetryData = {};
                beacons.forEach((beacon) => {
                    const uuid = beacon.uuid.toUpperCase();
                    telemetryData[`rssi_${uuid}`] = beacon.rssi;
                });
                uni.request({
                    url: 'http://140.143.139.183:8080/api/v1/23WFRgK4lx1qq5QvrbDp/telemetry',
                    method: 'POST',
                    ...
                    data: telemetryData,
                    success: (res) => {
                        console.log('RSSI 数据发送成功', res.data);
                    },
                    fail: (err) => {
                        console.error('RSSI 数据发送失败', err);
                    }
                });
            },
		},
